...
export default function CommunityPage() {
  ...
  const fetchPosts = async () => {
    if (!user?.userId || !isInitialized || !id) {
      console.log('CommunityPage: Нет userId, ID или инициализация не завершена, пропуск загрузки постов');
      return;
    }
    try {
  ...
  useEffect(() => {
    if (isInitialized && user) {
      ...
      if (id) {
        fetchCommunity().then(() => {
          return Promise.all([fetchPosts(), fetchFriends()]);
        }).then(() => {
          console.log('CommunityPage: Состояние после загрузки - community:', community);
          setLoading(false);
        }).catch((err) => {
        ...
        fetchCommunities().then(() => {
          ...
          console.log('CommunityPage: Список сообществ загружен, communities:', communities);
          setLoading(false);
        }).catch((err) => {
          console.error('Ошибка при загрузке списка сообществ:', err);
          setError('Ошибка загрузки списка сообществ');
          setLoading(false);
        });
      }
    }
            ...
            <ul>
              {friends.map((friend) => (
                <li key={friend._id} className="d-flex justify-content-between align-items-center">
                  {friend.username}
                  {community?.members && !community.members.some((m) => m._id === friend._id) && (
